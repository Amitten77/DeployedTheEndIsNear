...
function rectsIntersect(a: any, b: any) {
  ...
  const FACTOR = 0;
  return aBox.x + aBox.width+FACTOR > bBox.x && bBox.x + bBox.width+FACTOR > aBox.x
        && aBox.y + aBox.height+FACTOR > bBox.y && bBox.y + bBox.height+FACTOR > aBox.y
}
...
const pixiContainerRef = useRef<HTMLDivElement>(null);
    ...
    app.ticker.add((delta) =>  //GOATED FUNCTION
  {
      ...
      lassoCooldown += 1
      //Farmer Controls
      if (Math.abs(user.x - farmerStanding.x) > 20) { 
        if (user.x > farmerStanding.x) {
